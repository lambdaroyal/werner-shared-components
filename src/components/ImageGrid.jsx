...
export const ImageGrid = (props) => {
    const DownloadButton = (p) => {
        const downloadImage = (url, name) => async () => {
    ...
    const Carousel = (p) => {
        ...
        const [actionsAvailable, setActionsAvailable] = createSignal(false);
      ...
      <div class={clsx("w-full sm:w-96 flex scroll-snap", p.class)}>
        <div class="relative w-full group" onmouseenter={() => { setActionsAvailable(true); }} onmouseleave={() => { setActionsAvailable(false); }}>
          <img src={props.images[index()].source} class="w-full"/>
         ...
         style={`visibility: ${index() != 0 ? "initial" : "hidden"};`}>
              ...
              <button class="btn btn-circle no-animation" onClick={rightButtonClick} style={`visibility: ${index() < props.images.length - 1 ? "initial" : "hidden"};`}>
                ❯
              </button>
            </Show>
